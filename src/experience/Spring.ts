...
export class Spring {
  ...
  initVisuals() {
    ...
    const points = this.bodies.map(b => new THREE.Vector3(b.position.x, b.position.y, b.position.z));
    this.curve = new THREE.CatmullRomCurve3(points);
    this.curve.curveType = 'catmullrom';
    this.curve.tension = 0.5;

    // Tube geometry
    const geometry = new THREE.TubeGeometry(this.curve, 64, this.params.radius, 8, false);
  ...
  update() {
    ...
    this.bodies.forEach((body, i) => {
      this.curve.points[i].set(body.position.x, body.position.y, body.position.z);
    });

    // Update geometry
    this.mesh.geometry.dispose();
    this.mesh.geometry = new THREE.TubeGeometry(this.curve, 64, this.params.radius, 8, false);
  }
}
